...
def build_github_query(date: str, keywords_file: Optional[str] = None) -> str:
    ...
    keyword_query = " OR ".join([f'"{kw}"' for kw in source_keywords[:10]])  # Limit to avoid query too long
...
def matches_keywords(text: str, source: Optional[str] = None, keywords_file: Optional[str] = None) -> bool:
    """Check if text matches any keywords for the given source."""
    if not text:
        return False
    
    text_lower = text.lower()
    keywords = get_source_keywords(source, keywords_file) if source else get_core_keywords(keywords_file)
